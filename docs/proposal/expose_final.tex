...
\vspace{0.5em}
\noindent\textbf{Alternative titles (generic --- final title to be agreed with supervisor):}
\begin{itemize}[label=\textbullet, leftmargin=1.5em]
    \item Autonomous Mobile Manipulation: A Simulation-to-Reality Study on the LIMO Cobot
    \item Sim-to-Real Transfer for Autonomous Mobile Manipulation in On-Demand Manufacturing
    \item From Simulation to Reality: Autonomous Pick-Place-Stack with a Compact Mobile Manipulator
    \item Coordinated Navigation and Manipulation for Autonomous Box Stacking: A Sim-to-Real Approach
    \item Bridging Simulation and Reality in Autonomous Mobile Manipulation
    \item Autonomous Mobile Manipulation with Error-Aware Sim-to-Real Transfer
\end{itemize}
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=1.5em]
    ...
    platform?
    \item \textbf{Drift characterisation and error recovery.} How does position error
    accumulate over repeated navigation cycles, at what drift magnitude does the arm's
    reachable envelope fail to encompass the target object, and \emph{can the system
    detect this error (re-checking the object pose with the camera before each place)
    and recover --- correcting, retrying, or flagging for human intervention --- rather
...
\begin{enumerate}[label=\textbf{\arabic*.}]
    ...
    stack $\to$ return} cycles without human intervention. The place table is a
    \textbf{fixed, known station marked with an AprilTag fiducial}, giving the robot a
    precise, repeatable docking and drop reference; a \emph{verify-before-place} step
    re-checks the target pose with the camera before each placement.
    ...
    task failure mode.
    \item \textbf{Sim-to-Real Evaluation and Error Analysis} --- Deploy the validated
    pipeline on the physical LIMO Cobot. Compare simulation drift profiles (upper bound:
    perfect odom) against hardware drift profiles (real encoder odometry + AMCL) to
    quantify the sim-to-real gap and identify which assumptions the simulation makes that
    break on hardware. Crucially, analyse \emph{how the system handles error in
    practice}: log every placement failure, distinguish random from \emph{systematic}
    drift (errors that recur after a predictable number of iterations), and evaluate the
    verify-before-place recovery behaviour --- how reliably the robot detects a bad pose
    and corrects, retries, or hands off. The amount of validation performed on hardware
    versus simulation (a staged 50--100\,\% split) is a deliberate methodological choice
...
\begin{itemize}[label=\checkmark, leftmargin=1.5em]
    ...
    known layout, static obstacles). The map is built with \emph{distinct, asymmetric
    features} (not an empty or symmetric space) so that AMCL can localise reliably.
    ...
    \item \textbf{Fixed, fiducial-marked place station.} The place table is at a known
    position carrying an AprilTag marker; the robot docks and drops against this
    fiducial reference for precise, repeatable stacking. (The fixed-position assumption
    ...
    de-projection), with a \emph{verify-before-place} re-check of the target pose prior
    to each placement.
    \item Quantitative drift analysis: per-iteration position error and grasp success.
    \item Basic autonomous error handling: detection of failed/drifted placements and
    recovery by retry or human hand-off.
...
To make stacking precise and repeatable, the target table is a \textbf{fixed station
marked with an AprilTag fiducial}: the robot docks against this known reference and,
before each placement, re-checks the box pose with its camera (\emph{verify-before-place})
rather than placing blindly --- so that a navigation error does not silently propagate
into a misplaced box.
...
comparison of how well simulation predicts real performance. A second contribution is
the \textbf{error-handling analysis}: because real systems fail, the thesis characterises
\emph{how} the pipeline fails and recovers --- distinguishing random from systematic
drift, and evaluating whether the robot can detect a bad placement and correct, retry,
or request help. This reframes failure from a defect into a measured, analysable
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=1.5em, itemsep=0.3em]
    \item \textbf{Place stage} --- arm place motion (hover $\to$ descend $\to$ release
    $\to$ lift) is implemented and verified in isolation; remaining work is the
    \emph{docking calibration} so the base parks square in front of the place table
    rather than beside it.
    \item \textbf{Fiducial docking + verify-before-place} --- AprilTag detection on the
    fixed place table to close the docking loop precisely, and a camera re-check of the
    box pose before each placement.
    ...
    pick/place success, nav position error, detected box pose, and any error-recovery
    events.
    \item \textbf{Hardware deployment} --- swap Gazebo plugins for real hardware drivers,
    run the stacking cycles on the physical LIMO Cobot (staged 50--100\,\% hardware
