...
class BridgeNode(Node):
    ...
    def __init__(self):
        ...
        # Give odometry time to arrive before we start moving
        self._main_timer = self.create_timer(
            1.0, self._step, callback_group=self._cb_group)

    # ------------------------------------------------------------------ params

    def _declare_parameters(self):
        self.declare_parameter("spawn_x",  -8.95)
        self.declare_parameter("spawn_y",   0.0)
        self.declare_parameter("spawn_yaw", 0.0)

        self.declare_parameter("pick_zone_x",   0.60)
        self.declare_parameter("pick_zone_y",   0.0)
    ...
    def _read_parameters(self):
        ...
        self._spawn_x   = p("spawn_x").value
        self._spawn_y   = p("spawn_y").value
        self._spawn_yaw = p("spawn_yaw").value
    ...
    def _state_nav_to_pick(self):
        ...
        if int(elapsed) % 10 == 0:          # log every 10 s
            self.get_logger().info(
                f"NAV_TO_PICK: dist={dist:.2f} m  elapsed={elapsed:.0f} s  "
                f"timeout={self._current_nav_timeout:.0f} s  speed={speed} m/s"
            )

    # ── DETECT ───────────────────────────────────────────────────────────────

    def _state_detect(self):
        self._transition(State.DETECT)       # prevent re-entry
    ...
    def _state_pick(self):
        # Placeholder — replaced by MTC execution in full pipeline
        self.get_logger().info(
            f"Executing pick for object '{self._detected_object_id}'…  "
            f"(MTC planning_timeout={self._mtc_plan_timeout}s, "
            f"execution_timeout={self._mtc_exec_timeout}s)"
        )
        # TODO: call the MTC pick action here
        self._transition(State.NAV_TO_PLACE)
        self._nav_start_time = time.monotonic()
        self._current_nav_timeout = self._compute_nav_timeout(
            self._place_x, self._place_y, override=self._nav_to_place_timeout)
